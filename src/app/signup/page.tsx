...
export default function SignupPage() {
  ...
  const [form, setForm] = useState({ full_name: '', email: '', phone: '', whatsapp_number: '', password: '' });
      ...
      <div className="w-full space-y-4 rounded-2xl border border-border bg-card p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          {fields.map((f) => (
            <div key={f.key}>
              <Label>{f.label}</Label>
              <Input type={f.type} value={form[f.key]} onChange={(e) => setForm((prev) => ({ ...prev, [f.key]: e.target.value }))} className="mt-1" required placeholder={f.placeholder} />
            </div>
          ))}
          <Button disabled={loading} className="w-full">{loading ? 'Creating account...' : 'Create account'}</Button>
        </form>

        <p className="text-center text-sm text-muted-foreground">Already have an account? <Link href="/login" className="text-primary underline">Sign in</Link></p>
      </div>
    </div>
  );
}
