    ...
    <div className="mx-auto flex w-full max-w-lg flex-1 items-center px-4 py-10">
      <div className="w-full space-y-4 rounded-2xl border border-border bg-card p-6">
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Sign in to SOV Connect</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          {message ? <p className="rounded bg-destructive/10 px-3 py-2 text-sm text-destructive">{message}</p> : null}
          <div><Label>Email</Label><Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="mt-1" required placeholder="you@email.com" /></div>
          <div><Label>Password</Label><Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="mt-1" required /></div>
          <Button disabled={loading} className="w-full">{loading ? 'Signing in...' : 'Sign in'}</Button>
        </form>

        <p className="text-center text-sm text-muted-foreground">Need an account? <Link href="/signup" className="text-primary underline">Sign up</Link></p>
      </div>
    </div>
  );
}
