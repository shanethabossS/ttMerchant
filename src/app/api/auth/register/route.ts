...
import { createLocalUser } from '@/lib/intake/store';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      const res = NextResponse.json({ token, user_id: user.id, email: user.email, full_name: user.full_name }, { status: 201 });
      res.cookies.set('auth_token', token, { httpOnly: true, secure: SECURE_COOKIE, sameSite: 'lax', maxAge: COOKIE_MAX_AGE, path: '/' });
      return res;
    } catch (error) {
      return NextResponse.json({ error: error instanceof Error ? error.message : 'Auth service unavailable' }, { status: 503 });
    }
  }
}
