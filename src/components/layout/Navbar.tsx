...
export function Navbar() {
  ...
  return (
    <nav className={`${fraunces.variable} sticky top-0 z-50 w-full border-b border-border bg-background/92 backdrop-blur`}>
      ...
      <div className="container mx-auto flex min-h-16 items-center justify-between gap-4 px-4 py-3">
        <Link href="/" className="flex min-w-0 items-center gap-3">
          <span className="flex size-10 shrink-0 items-center justify-center rounded-sm border border-foreground bg-foreground text-sm font-black text-background">
            LT
          </span>
          <span className="min-w-0">
            <span className="lt-serif block truncate text-xl font-semibold tracking-tight">LaunchTT</span>
        ...
        <div className="hidden items-center gap-2 md:flex">
          <ThemeToggle />
          <div className="hidden items-center gap-1.5 rounded-sm border border-border px-3 py-1 text-xs font-bold text-foreground lg:flex">
            <ShieldCheck className="size-3.5" style={{ color: 'color-mix(in oklab, var(--accent) 80%, var(--foreground))' }} />
            Secure
          </div>
          {!loading && user ? (
            ...
            <div className="flex gap-2">
              <Link href="/login"><Button variant="outline" size="sm" className="rounded-sm">Sign in</Button></Link>
              <Link href="/signup"><Button size="sm" className="rounded-sm">Sign up</Button></Link>
            </div>
          ) : null}
        </div>
